...
class Profile extends React.Component {
  ...
  checkFollow = (auth, user) => {
    return user.followers.findIndex(follower => follower._id === auth.user_id) > -1;
  }
  ...
  handleToggleLike = post => {
    ...
    sendRequest(post._id)
      .then(postData => {
        ...
        const updatedPosts = [...this.state.posts.slice(0, postIndex),
                              postData,
  ...
  render() {
    ...
    const { isLoading, avatar, isAuth, user, isFollowing, posts, isDeletingPost } = this.state;
    ...
    return (
      ...
      gutterBottom>
        ...
        {isLoading ? (
            ...
            <CircularProgress className={classes.progress} size={55} tickness={5}/></div>) : (
                <List dense>
                <ListItem>
                <ListItemAvatar>
              ...
              ) : (<div><FollowUser isFollowing={ isFollowing } toggleFollow={this.toggleFollow}/></div>)}
              </ListItem>
                <Divider/>
                <ListItem>
              ...
              {/* Display User's Posts, Following, and Followers */}
                <ProfileTabs user={user} auth={auth} posts={posts} isDeletingPost={isDeletingPost} handleDeletePost={this.handleDeletePost} handleToggleLike={this.handleToggleLike} handleAddComment={this.handleAddComment} handleDeleteComment={this.handleDeleteComment}/>
              </List>
            )}
      </Paper>)
  }
